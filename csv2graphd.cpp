...
void getDivide(std::set<std::string> &twoHopResults, std::string filename, std::string outdir, std::set<std::string>& comment, std::set<std::string>& post, std::map<std::string, std::map<std::string,std::string>>& knows) {
    ...
    // file.open(filename);
    // file2.open(directory1+"person_likes_comment_0_0.csv");
    ...
    // file2.open(directory1+"person_likes_post_0_0.csv");
    ...
    file.open(directory+"/person_email_emailaddress_0_0.csv");
    file2.open(directory1+"/person_email_emailaddress_0_0.csv");
    // std::set <std::string> newPerson;
    ...
    file.open(directory+"/person_speaks_language_0_0.csv");
    file2.open(directory1+"/person_speaks_language_0_0.csv");
    // std::set <std::string> newPerson;
...
int main() {
    ...
    std::ifstream file("./input/ldbc/test01/dynamic/person_0_0.csv");
    std::string line;
    ...
    std::ifstream file2("./input/ldbc/test01/dynamic/person_knows_person_0_0.csv");
    ...
    //     std::cout<<"smallest onehop:"<<getSmallestOneHop(cNeighbor,neighbor_counts)<<std::endl;
    //     std::cout<<"smallest twohop:"<<getSmallestOneHop(cNeighbor,two_hop_neighbor_counts)<<std::endl;
        
    //     std::cout<<"bigest onehop:"<<getBiggestOneHop(cNeighbor,neighbor_counts)<<std::endl;

    //     // getPerson(twoHopResults, "./0/person_0_0.csv", people);

    //     std::set<std::string> comment;
    //     std::set<std::string> post;

    //     getDivide(twoHopResults, "./input/ldbc/test01/dynamic/", "./output/"+givenPersonID+"/", comment, post, knows);
    ...
    //     }
    //     while(threshhold<10000&&cNeighbor.size()!=0) {
    ...
    // std::set<std::string> testth;
    // for(auto i: twoHopSets) {
    //     for(auto j: i) {
    //         testth.insert(j);
    //     }
    // }
    // std::cout<<"testth"<<testth.size()<<std::endl;
    
    // 合并相似度高的两跳person集
    double merge_threshold = 0.7;  // 相似度阈值
    ...
    //     }
    // }
    // // 合并后的总人数怎么还变少了呢？已修复,合并删除存在问题
    // std::cout<<"twoHopSets num:"<<tset.size()<<std::endl;
    // std::cout<<"mergedSets num:"<<mset.size()<<std::endl;
